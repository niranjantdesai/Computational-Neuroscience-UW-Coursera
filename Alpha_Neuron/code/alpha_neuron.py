...
# t_peak = 1 # ms
t_peak = np.arange(0.5, 10.5, 0.5)
g_peak = 0.05 # nS (peak synaptic conductance)

spike_cnt = np.zeros([t_peak.size])
for i in range(t_peak.size):
    const = g_peak / (t_peak[i]*np.exp(-1));
    t_vec = np.arange(0, t_a + h, h)
    alpha_func = const * t_vec * (np.exp(-t_vec/t_peak[i]))
...
# Plot output spike count as a function of t_peak
plt.plot(t_peak, spike_cnt)
plt.xlabel('t_peak (in ms)')
plt.ylabel('Output spike count')

"""
Increasing t_peak decreases the decay rate of the synaptic conductance,
allowing more charge to flow per spike and increasing the extent of summation
between spikes
"""